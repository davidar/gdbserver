...
struct reg_struct regs_map[] = {
    ...
    {30, 4}, {31, 4}, {32, 4}, {33, 4}, {34, 4}, {35, 4}, {36, 4}, {37, 4}, {38, 4},
};
...
uint8_t *inbuf_get() { return in.buf; }

int inbuf_end() { return in.end; }
...
void inbuf_erase_head(ssize_t end) { pktbuf_erase_head(&in, end); }

void pktbuf_clear(struct packet_buf *pkt) { pkt->end = 0; }
...
static bool poll_incoming(int sock_fd) { return poll_socket(sock_fd, POLLIN); }

static void poll_outgoing(int sock_fd) { poll_socket(sock_fd, POLLOUT); }
...
void write_data_raw(const uint8_t *data, ssize_t len) { pktbuf_insert(&out, data, len); }
...
void sigint_pid() { kill(-threads.t[0].pid, SIGINT); }

bool is_clone_event(int status) { return (status >> 8 == (SIGTRAP | (PTRACE_EVENT_CLONE << 8))); }
...
void prepare_resume_reply(uint8_t *buf, bool cont) {
  ...
  if (WIFSTOPPED(threads.curr->stat)) {
    if (cont)
      stop_threads();
    sprintf(buf, "T%02xthread:p%02x.%02x;", WSTOPSIG(threads.curr->stat), threads.curr->pid,
            threads.curr->tid);
  }
}
